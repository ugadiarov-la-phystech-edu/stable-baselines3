...
class EPG(A2C):
    def __init__(
        ...
        self.detach_q_values = detach_q_values
        if not policy_kwargs.get('use_q_critic', False):
            raise ValueError(f'It is mandatory for {type(self)}')
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(batch_size=None):
            ...
            policy_result = self.policy.evaluate_actions(rollout_data.observations, actions)
            q_values, values, entropy = policy_result['q_values'], policy_result['values'], policy_result['entropy']
            ...
            policy_loss = -th.mean(values)

            # Value loss using the TD(gae_lambda) target
            value_loss = F.mse_loss(q_values, rollout_data.returns)
