...
from stable_baselines3.sac.policies import SACPolicy, SACWMPolicy, SACWMGNNPolicy
...
class SACWM(OffPolicyAlgorithm):
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(SACWM, self)._excluded_save_params() + ["actor", "critic", "critic_target"]
...
class SACWMGNN(OffPolicyAlgorithm):
    def __init__(
            self,
            policy,
            env: Union[GymEnv, str],
        ...
        # Complete graph underlying GNN
        self.moving_boxes = torch.ones(self.batch_size, self.policy.actor.num_objects, dtype=torch.float32,
                                       device=self.device)
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            actions_pi, log_prob = self.actor.action_log_prob(
                [features, self.fake_actions, self.moving_boxes, False])
            log_prob = log_prob.reshape(-1, 1)
            ...
            with th.no_grad():
                ...
                next_actions, next_log_prob = self.actor.action_log_prob(
                    [next_features_detach, self.fake_actions, self.moving_boxes, False])
                # Compute the next Q values: min over all critics targets
                next_q_values = th.stack(
                    self.critic_target(next_features_detach, next_actions, self.moving_boxes, self.actor),
                    dim=1)
                next_q_values, _ = th.min(next_q_values, dim=1, keepdim=True)
                # add entropy term
                next_q_values = next_q_values - ent_coef * next_log_prob.reshape(-1, 1)
            ...
            # using action from the replay buffer
            features_detach = features.detach()
            current_q_values = self.critic(features_detach, replay_data.actions, self.moving_boxes,
                                           self.actor)
            ...
            # Alternative: actor_loss = th.mean(log_prob - qf1_pi)
            # Mean over all critic networks
            q_values_pi = th.stack(self.critic(features, actions_pi, self.moving_boxes, self.actor), dim=1)
            min_qf_pi, _ = th.min(q_values_pi, dim=1, keepdim=True)
            actor_loss = (ent_coef * log_prob - min_qf_pi).mean()
            actor_losses.append(actor_loss.item())
    ...
    def predict(
        ...
        embedding = self.policy.extract_features(obs=torch.as_tensor(observation, dtype=torch.float32))
        batch_size = observation.shape[0]
        action = self.fake_actions[:batch_size]
        boxes = self.moving_boxes[:batch_size]
        return self.policy.actor.predict([embedding, action, boxes, False], state, episode_start, deterministic)
...
class SACGNN(OffPolicyAlgorithm):
    def __init__(
            self,
            policy,
            env: Union[GymEnv, str],
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(SACGNN, self)._excluded_save_params() + ["actor", "critic", "critic_target"]
