...
from stable_baselines3.treeqn.utils.pytorch_utils import View, nn_init
from stable_baselines3.treeqn.utils.einsum import einsum
from stable_baselines3.treeqn.models.transitions import build_transition_fn, MLPRewardFn
from stable_baselines3.treeqn.models.encoding import atari_encoder, push_encoder, navigation_encoder
from stable_baselines3.treeqn.utils.pytorch_utils import logsumexp
...
class TreeQNPolicy(nn.Module):
    def __init__(self,
        ...
        if input_mode in ("atari", "grafter", "crafter"):
            encoder = atari_encoder(ob_shape[0])
            dummy = Variable(torch.zeros(1, *ob_shape))
            conv_dim_out = tuple(encoder(dummy).size())[1:]
            self.obs_scale = 255.0
        elif input_mode == "push":
            encoder = push_encoder(ob_shape[0])
            dummy = Variable(torch.zeros(1, *ob_shape))
            conv_dim_out = tuple(encoder(dummy).size())[1:]
        elif input_mode == "navigation":
            encoder = navigation_encoder(ob_shape[0])
            dummy = Variable(torch.zeros(1, *ob_shape))
            conv_dim_out = tuple(encoder(dummy).size())[1:]
        else:
            raise ValueError("Input mode not accepted. use atari, push, navigation")
    ...
    def obs_to_variable(self, ob, volatile=False):
        ...
        return ob.float() / self.obs_scale
