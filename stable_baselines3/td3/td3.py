...
from stable_baselines3.td3.policies import TD3Policy, TD3WMPolicy
...
class TD3WM(OffPolicyAlgorithm):
    """
    Twin Delayed DDPG (TD3)
    Addressing Function Approximation Error in Actor-Critic Methods.
    ...
    def __init__(
        self,
        policy: Union[str, Type[TD3WMPolicy]],
        env: Union[GymEnv, str],
        learning_rate: Union[float, Schedule] = 1e-3,
        buffer_size: int = 1_000_000,  # 1e6
        learning_starts: int = 100,
        ...
        policy_kwargs['target_policy_noise'] = target_policy_noise
        policy_kwargs['target_noise_clip'] = target_noise_clip

        super(TD3WM, self).__init__(
            policy,
            env,
            TD3Policy,
            learning_rate,
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                # Compute the next Q-values: min over all critics targets
                next_q_values = th.cat(self.critic_target(replay_data.next_observations, next_actions, self.actor_target, use_noise=True), dim=1)
                next_q_values, _ = th.min(next_q_values, dim=1, keepdim=True)
                target_q_values = replay_data.rewards + (1 - replay_data.dones) * self.gamma * next_q_values

            # Get current Q-values estimates for each critic network
            current_q_values = self.critic(replay_data.observations, replay_data.actions, self.actor, use_noise=False)
            ...
            transition_loss = sum([F.mse_loss(next_states_prediction, next_states) for next_states_prediction in next_states_predictions])
            ...
            reward_loss = sum([F.mse_loss(rewards_prediction, replay_data.rewards) for rewards_prediction in rewards_predictions])
            ...
            if self._n_updates % self.policy_delay == 0:
                # Compute actor loss
                actor_loss = -self.critic.q1_forward(replay_data.observations, self.actor(replay_data.observations), self.actor).mean()
                actor_losses.append(actor_loss.item())
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(TD3WM, self)._excluded_save_params() + ["actor", "critic", "actor_target", "critic_target"]
